   ...
   /**
    ...
    MouseHit.prototype.__createMouse = function() {
	   ...
	   for(i=0;i<3;i++)
	   {
            ...
            for(j=0;j<3;j++)
			{
			  ...
			  mouse0.init('mouse1',this.mouseX[i][j],this.mouseY[i]-20,142);
			  mouse1.init('mouse2',this.mouseX[i][j],this.mouseY[i]-20,142);
			  mouse2.init('mouse3',this.mouseX[i][j],this.mouseY[i]-20,142);	  
              mouse3.init('mouse4',this.mouseX[i][j],this.mouseY[i]-20,142);
			  mouse4.init('mouse5',this.mouseX[i][j],this.mouseY[i]-18,120);
    ...
    MouseHit.prototype.ready = function(self,index) {
		...
		switch(index){
			...
			case 0:self.context.drawImage(self.startNumber,449,296,51,87,290,500,51,87);Audio.play('begin_music');break;    //绘制3
			case 1:self.context.drawImage(self.startNumber,390,296,54,87,290,500,54,87);Audio.play('begin_music');break;    //绘制2
			case 2:self.context.drawImage(self.startNumber,329,296,52,87,290,500,52,87);Audio.play('begin_music');break;    //绘制1
			case 3:self.context.drawImage(self.startNumber,18,293,186,85,175,500,293,87);Audio.play('second_music');break;   //绘制start
	...
	MouseHit.prototype.__checkIsPass = function(self){
	    ...
	    if(self.timeCaculator>900 || window.to_stop){             //关卡时间到
		   ...
		   self.__drawTimeRoller(self,self.timeCaculator/10); //绘制计时条
            var _timetxt=Math.floor(30-this.timeCaculator/30);//计算剩余时间
           document.getElementById('timetxt').innerHTML = _timetxt;//显示时间
		   self.timeCaculator++;
	...
	MouseHit.prototype.drawScreen = function(self){
		...
		for(j=0;j<3;j++) 
	 	...
	 	self.context.drawImage(self.bg_holeHide.image,0,410,self.bg_holeHide.width,80, 0, 410, self.bg_holeHide.width,80);   //第一行遮掩洞
        for(j=0;j<3;j++)
		  self.drawMouse(self,1,j); //第二行地鼠
		for(j=0;j<3;j++) 
		  self.star[1][j].draw(self.context);  //第二行星星
        self.context.drawImage(self.bg_holeHide.image,0,629,self.bg_holeHide.width,80, 0, 629, self.bg_holeHide.width,80); //第二行遮掩洞 
        for(j=0;j<3;j++)
		  self.drawMouse(self,2,j); //第三行地鼠
		for(j=0;j<3;j++) 
		  self.star[2][j].draw(self.context);  //第三行星星
		self.context.drawImage(self.bg_holeHide.image,0,845,self.bg_holeHide.width,80, 0, 845, self.bg_holeHide.width,80); //第三行遮掩洞 	
		for(i=0;i<3;i++) 
		  for(j=0;j<3;j++) 
